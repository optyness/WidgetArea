...
void CardList::setDataToWidget(QWidget *item, QAbstractItemModel *model, int row)
{
    ...
    case WType::NameCard:{
        ...
        data_widget->setName(model->data(model->index(row,0),Qt::UserRole+1).toString());
        data_widget->setFName(model->data(model->index(row,0),Qt::UserRole+2).toString());
        data_widget->setAge(model->data(model->index(row,0),Qt::UserRole+3).toInt());
        return;
    }
    case WType::Message:{
        ListWidgetItem *data_widget = qobject_cast<ListWidgetItem*>(item);
        data_widget->setSender(model->data(model->index(row,0),Qt::UserRole+1).toString());
        data_widget->setMessage(model->data(model->index(row,0),Qt::UserRole+2).toString());
        return;
    }
    default:{
        return;
